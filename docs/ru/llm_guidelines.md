...
> 1. В `PyJWT 2.x+` поле `"sub"` должно быть строкой. При создании токена обязательно используй `str(user_id)`.
> 2. Роуты API всегда должны возвращать ошибку в ключе `error` (а не `message`), так как `Response.error` по умолчанию формирует `{"error": "...", "code": "..."}`.
